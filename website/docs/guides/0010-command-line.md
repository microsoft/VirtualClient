...
| --packages, --package-store=\<connection\>                  | No          | string/connection string/SAS | A full connection description for an [Azure Storage Account](/docs/guides/integration-blob-storage) from which to download workload and dependency packages. This is required for most workloads because the workload binary/script packages are not typically packaged with the Virtual Client application itself. This option defaults to a public storage account VC team maintains.<br/><br/>The following are supported identifiers for this option:<br/><ul><li>Storage Account blob service SAS URIs</li><li>Storage Account blob container SAS URIs</li><li>Microsoft Entra ID/Apps using a certificate</li><li>Microsoft Azure managed identities</li></ul>See [Azure Storage Account Integration](/docs/guides/integration-blob-storage) for additional details on supported identifiers.<br/><br/><mark>Always surround connection descriptions with quotation marks.</mark> |
...
* ### bootstrap
  ...
  * **Certificate Bootstrapping**:  
  ...
  | --packages, --package-store=\<connection\>                | Conditional | string/connection string/SAS | Connection description for an Azure Storage Account/container to download packages from. See [Azure Storage Account Integration](/docs/guides/integration-blob-storage). |
  | -c, --clean, --clean=\<target,target...\>                 | No          | string                       | Instructs the application to perform an initial clean before continuing to remove pre-existing files/content created by the application from the file system. This can include log files, packages previously downloaded, state management and temporary files. This option can be used as a flag (e.g. --clean) as well to clean all file content. Valid target resources include: logs, packages, state, temp, all (e.g. --clean=logs, --clean=packages). Multiple resources can be comma-delimited (e.g. --clean=logs,packages). To perform a full reset of the application state, use the option as a flag (e.g. --clean). This effectively sets the application back to a "first run" state. |
  ...
  | --event-hub=\<connection\>                                | No          | string/connection string     | Event Hub connection for telemetry upload (note this option can also be defined with `--logger="eventhub;{connection}"`). |
...
* ### upload-files
  ...
  | Option                                                          | Required    | Data Type                    | Description |
  |-----------------------------------------------------------------|-------------|------------------------------|-------------|
  | --content, --content-store=\<connection\>                       | Yes         | string/connection string/SAS | A full connection description for an [Azure Storage Account](/docs/guides/integration-blob-storage) to use for uploading files/content (e.g. log files).<br/><br/>The following are supported identifiers for this option:<br/><ul><li>Storage Account blob service SAS URIs</li><li>Storage Account blob container SAS URIs</li><li>Microsoft Entra ID/Apps using a certificate</li><li>Microsoft Azure managed identities</li></ul>See [Azure Storage Account Integration](/docs/guides/integration-blob-storage) for additional details on supported identifiers.<br/><br/><mark>Always surround connection descriptions with quotation marks.</mark> |
...
* ### upload-telemetry
  ...
  [External Telemetry Schema and Requirements](/docs/guides/telemetry)
  ...
  | --event-hub=\<connection\>                 | Yes         | string/connection string     | Event Hub connection for telemetry upload. |
...
* **Comma-Separated Value (CSV) Logger**  
  ...
  [Telemetry Support](/docs/guides/telemetry)

  ``` bash
  # Specify the logger on the command line.
...
* **Event Hub Logger**  
  ...
  [Event Hub Integration](/docs/guides/integration-event-hub)

  ``` bash
  # Specify the logger on the command line. Note that the URI connection to the Event Hub must be specified 
  # as shown below.
  # Format: --logger="eventhub;{connection}"
  VirtualClient.exe --profile=PERF-CPU-OPENSSL.json --logger="eventhub;sb://anynamespace.servicebus.windows.net?cid=...&tid=..."
  ```

* **Delimited-JSON File Logger**  
  This logger writes delimited JSON text entries to the local disk in the "logs" folder.

  ``` bash
  # Specify the logger on the command line.
  VirtualClient.exe --profile=PERF-CPU-OPENSSL.json --logger=file
  ```
...
* **Custom-Developed/Extensions Logger**  
  Developers writing extensions to the Virtual Client can define and provide custom loggers to the application. These loggers are available
  as part of the [.NET extensions development process](/docs/developing/develop-extensions) for Virtual Client. The assemblies/.dlls are loaded at runtime allowing the
  user to reference the logger by namespace and class name.
  
  ``` bash
  # Specify the logger on the command line.
  # Format: --logger="{Namespace}.{ClassName};Parameter1=Value1;Parameter2=Value2"
  VirtualClient.exe --profile=EXAMPLE-WORKLOAD.json --logger="Any.VirtualClient.Extensions.CustomLogger;LogDirectory=./logs/custom_logger"
  ```
