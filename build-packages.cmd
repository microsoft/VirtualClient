...
for %%a in (%*) do (
    ...
    if /i "%%a" == "+sdk-agent" (
        set PACKAGE_SDK_AGENT=true
    )

    if /i "%%a" == "#sdk-agent" (
        set PACKAGE_SDK_AGENT=true,only
    )
)
...
if /i not "%PACKAGE_SDK_AGENT%" == "true,only" (
    echo:
...
if /i "%PACKAGE_SDK_AGENT:~0,4%" == "true" (
    echo:
    echo [Create NuGet Package] sdk_agent.%PACKAGE_VERSION%
    echo ----------------------------------------------------------
    call dotnet pack %PACKAGES_PROJECT% --force --no-restore --no-build -c %BUILD_CONFIGURATION% -v Detailed ^
    -p:Version=%PACKAGE_VERSION% -p:NuspecFile=%PACKAGE_DIR%\nuspec\SdkAgent.nuspec && echo: || Goto :Error

    call ren "%SCRIPT_DIR%\out\packages\SdkAgent*.nupkg" sdk_agent*.zip && echo: || Goto :Error
)
...
echo Usage:
echo ---------------------
echo build-packages.cmd [--suffix ^<alpha^|beta^>] [+sdk-agent^|#sdk-agent]
echo:
echo Examples:
echo ---------------------
...
echo %SCRIPT_DIR%^> build-packages.cmd
echo:
echo # Include SDK Agent packages:
echo %SCRIPT_DIR%^> build.cmd +sdk-agent
echo %SCRIPT_DIR%^> build-packages.cmd +sdk-agent
echo:
echo # Build the SDK Agent packages only:
echo %SCRIPT_DIR%^> build.cmd #sdk-agent
echo %SCRIPT_DIR%^> build-packages.cmd #sdk-agent
echo:
Goto :Finish
