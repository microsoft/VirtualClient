    ...
    internal class ExecuteRemoteAgentCommand : ExecuteProfileCommand
    {
        ...
        protected string GetTargetCommandArguments(IEnumerable<Token> commandLineTokens)
        {
            ...
            if (commandLineTokens?.Any() == true)
            {
                ...
                IEnumerable<string> optionsAlwaysExcluded = new HashSet<string>(targetOption.Aliases);
                ...
                for (int t = 0; t < commandTokens.Count() - 1; t++)
                {
                    ...
                    if (token.Type == TokenType.Option && !optionsAlwaysExcluded.Contains(token.Value))
                    {
                        ...
                        {
                            targetCommandArguments.Add($"{token.Value}=\"{nextToken.Value?.Trim()}\"");
                        }
                        ...
                        {
                            targetCommandArguments.Add(token.Value?.Trim());
                        }
                    }
                }
                ...
                {
                    targetCommandArguments.Add(lastToken.Value?.Trim());
                }
            }
