...
namespace VirtualClient
{
    ...
    public sealed class Program
    {
        ...
        internal static CommandLineBuilder SetupCommandLine(string[] args, CancellationTokenSource cancellationTokenSource)
        {
            ...
            if (!Program.HasTargetAgentOptions(args))
            {
                rootCommand = new RootCommand("Executes workload and monitoring profiles on the system.")
                {
                    // OPTIONAL
                    // -------------------------------------------------------------------
                     // --profile
                    ...
                    // --ssh
                    OptionFactory.CreateSshOption(required: false),
                ...
                remoteSubcommand.Handler = CommandHandler.Create<ExecuteOnAgentCommand>(cmd => cmd.ExecuteAsync(args, cancellationTokenSource));
            ...
            {
                rootCommand = new RootCommand("Executes the application as a controller targeting a set of agents via SSH connections.")
                {
                    // REQUIRED
                    // -------------------------------------------------------------------
                    // --agent
                    OptionFactory.CreateAgentOption(required: true),
                ...
                rootCommand.AddArgument(OptionFactory.CreateCommandArgument(required: false, string.Empty));
                rootCommand.TreatUnmatchedTokensAsErrors = false;
                rootCommand.Handler = CommandHandler.Create<ExecuteControllerCommand>(cmd => cmd.ExecuteAsync(args, cancellationTokenSource));
            }
        ...
        private static Command CreateRemoteSubcommand(DefaultSettings settings)
        {
                ...
                "Executes workload and monitoring profiles on a remote/target system through an SSH connection.")
            {
                ...
                // -------------------------------------------------------------------
                // --ssh
                OptionFactory.CreateSshOption(required: true),
        ...
        private static bool HasTargetAgentOptions(string[] args)
        {
            bool hasTargets = false;
            Option agentOption = OptionFactory.CreateAgentOption(false);
            Regex agentExpression = new Regex(string.Join("|", agentOption.Aliases.Select(a => $"{a}=")));

            foreach (string arg in args)
            {
                if (agentExpression.IsMatch(arg))
                {
                    hasTargets = true;
                    break;
                }
