            ...
            try
            {
                ...
                using (CancellationTokenSource cancellationSource = VirtualClientRuntime.CancellationSource)
                {
                    try
                    {
                        ...
                        string[] preprocessedArgs = null;

#if !SDK_AGENT
                        preprocessedArgs = CommandFactory.PreprocessArguments(args);
                        commandBuilder = CommandFactory.CreateCommandBuilder(preprocessedArgs, cancellationSource);
                        
#else
                        preprocessedArgs = CommandFactory.SdkAgent.PreprocessArguments(args);
                        commandBuilder = CommandFactory.SdkAgent.CreateCommandBuilder(preprocessedArgs, cancellationSource);
#endif
