            ...
            try
            {
                ...
                using (CancellationTokenSource cancellationSource = VirtualClientRuntime.CancellationSource)
                {
                    try
                    {
                        ...
                        // Note that we build 2 different versions of the Virtual Client each with both similar and
                        // different command lines. The SDK agent is a build of Virtual Client that supports controller/agent
                        // scenarios. As a controller, the SDK agent can execute workflows on remote/target systems.
#if !SDK_AGENT
...
#endif
                        ParseResult parseResults = commandBuilder.ParseArguments(args, out IEnumerable<Token> commandLineTokens);
                        Program.CommandLineTokens = commandLineTokens;
