    ...
    public class ZypperPackageInstallation : VirtualClientComponent
    {
        ...
        protected override async Task ExecuteAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            await this.InstallRetryPolicy.ExecuteAsync(async () =>
            {
                ...
                using (IProcessProxy process = await this.ExecuteCommandAsync(ZypperPackageInstallation.ZypperCommand, $"update", Environment.CurrentDirectory, telemetryContext, cancellationToken, runElevated: true)
                ...
                using (IProcessProxy process = await this.ExecuteCommandAsync(ZypperPackageInstallation.ZypperCommand, formattedArguments, Environment.CurrentDirectory, telemetryContext, cancellationToken, runElevated: true)
                    .ConfigureAwait(false))
                {
                    if (!cancellationToken.IsCancellationRequested)
                    {
        ...
        {
            using (IProcessProxy process = await this.ExecuteCommandAsync(ZypperPackageInstallation.ZypperCommand, $"list {packageName}", Environment.CurrentDirectory, telemetryContext, cancellationToken, runElevated: true)
                    .ConfigureAwait(false))
            {
                if (!cancellationToken.IsCancellationRequested)
                {
