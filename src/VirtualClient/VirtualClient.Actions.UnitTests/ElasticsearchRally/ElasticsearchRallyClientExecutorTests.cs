...
namespace VirtualClient.Actions
{
    ...
    public class ElasticsearchRallyClientExecutorTests : MockFixture
    {
        ...
        public void RallyChallengePropertyReturnsExpectedValue()
        {
            ...
            this.Parameters[nameof(ElasticsearchRallyClientExecutor.RallyCommandLineArguments)] = "--challenge=append-no-conflicts";

            using (TestElasticsearchRallyClientExecutor executor = new TestElasticsearchRallyClientExecutor(this.Dependencies, this.Parameters))
            {
                Assert.AreEqual("--challenge=append-no-conflicts", executor.RallyCommandLineArguments);
            }
        }
        ...
        public void RallyIncludeTasksPropertyReturnsExpectedValue()
        {
            ...
            this.Parameters[nameof(ElasticsearchRallyClientExecutor.RallyCommandLineArguments)] = "--include-tasks=index,search";

            using (TestElasticsearchRallyClientExecutor executor = new TestElasticsearchRallyClientExecutor(this.Dependencies, this.Parameters))
            {
                Assert.AreEqual("--include-tasks=index,search", executor.RallyCommandLineArguments);
            }
        }
        ...
        public async Task RallyCommandLineArgumentsOverridesDefaultCommandWhenSpecified()
        {
            ...
            string customCommand = "--custom-flag";
            this.Parameters[nameof(ElasticsearchRallyClientExecutor.RallyTrackName)] = "custom";
            this.Parameters[nameof(ElasticsearchRallyClientExecutor.RallyCommandLineArguments)] = customCommand;
            ...
            using (TestElasticsearchRallyClientExecutor executor = new TestElasticsearchRallyClientExecutor(this.Dependencies, this.Parameters))
            {
                executor.OnRunCommand = (command, arguments) =>
                {
                    if (command == "/usr/bin/sudo" && arguments.Contains("python3 -m pipx run esrally race --track=custom") && arguments.Contains("--custom-flag"))
                    {
