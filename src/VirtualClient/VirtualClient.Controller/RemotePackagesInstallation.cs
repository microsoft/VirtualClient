...
namespace VirtualClient.Controller
{
    ...
    public class RemotePackagesInstallation : VirtualClientControllerComponent
    {
            ...
            return this.Logger.LogMessageAsync($"{nameof(this.TypeName)}.ExecuteOnTarget", telemetryContext, async () =>
            {
                try
                {
                    ...
                    if (!cancellationToken.IsCancellationRequested)
                    {
                        ...
                        await this.InstallPackagesAsync(targetAgent, targetPlatformSpecifics, telemetryContext, cancellationToken);
                    }
                }
                finally
                {
        ...
        /// </summary>
        /// <param name="targetAgent">The SSH client to interface with the remote system.</param>
        ...
        /// <param name="telemetryContext">Provides context information to include in telemetry output.</param>
        /// <param name="cancellationToken">A token that can be used to cancel the operation.</param>
        protected async Task InstallPackagesAsync(ISshClientProxy targetAgent, PlatformSpecifics targetPlatformArchitecture, EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            if (!cancellationToken.IsCancellationRequested)
            {
                ...
                ConsoleLogger.Default.LogMessage($"Copy Packages To: {targetAgent.ConnectionInfo.Host}", telemetryContext);
                await this.CopyDirectoryToAsync(targetAgent, sourcePackagesPath, targetPackagesInstallationPath, telemetryContext, cancellationToken, force: true);

                // e.g.
                // chmod -R 2777 {target_path}
                await this.SetFilePermissionsAsync(targetAgent, targetPackagesInstallationPath, targetPlatformArchitecture.Platform, cancellationToken);
            }
        }
