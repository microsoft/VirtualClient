    ...
    public class ExecuteSshCommand : VirtualClientControllerComponent
    {
        ...
        protected override Task ExecuteAsync(ISshClientProxy targetAgent, EventContext telemetryContext, CancellationToken cancellationToken)
        {
            return this.Logger.LogMessageAsync($"{nameof(this.TypeName)}.ExecuteOnTarget", telemetryContext, async () =>
            {
                ...
                try
                {
                    await targetAgent.ConnectAsync();
                    ProcessDetails result = await targetAgent.ExecuteCommandAsync(this.Command, cancellationToken);

                    if (!cancellationToken.IsCancellationRequested)
                    {
                        ConsoleLogger.Default.LogTraceMessage(result.ToString());
