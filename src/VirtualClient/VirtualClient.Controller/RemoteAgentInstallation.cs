                ...
                try
                {
                    ...
                    if (!cancellationToken.IsCancellationRequested)
                    {
                        ...
                        if (!cancellationToken.IsCancellationRequested)
                        {
                            ...
                            ConsoleLogger.Default.LogMessage($"Copy Agent To: {targetAgent.ConnectionInfo.Host}", telemetryContext);
                            await this.CopyDirectoryToAsync(targetAgent, agentPackagePath, targetInstallationPath, telemetryContext, cancellationToken, force: true);

                            // e.g.
                            // chmod -R 2777 {target_path}
                            await this.SetFilePermissionsAsync(targetAgent, targetInstallationPath, targetPlatform, cancellationToken);
                        }
                    }
                }
                finally
