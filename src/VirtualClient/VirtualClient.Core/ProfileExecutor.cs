...
namespace VirtualClient
{
    ...
    public class ProfileExecutor : IDisposable
    {
            ...
            using (CancellationTokenSource tokenSource = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken))
            {
                ...
                if (!profileCancellationToken.IsCancellationRequested)
                {
                    ...
                    if (this.ExecuteActions || this.ExecuteMonitors)
                    {
                        ...
                        if (timing.Timeout != null && timing.IsTimedOut)
                        {
                            ...
                            this.Logger.LogMessage($"{nameof(ProfileExecutor)}.ExecutionTimeout", LogLevel.Information, timeoutContext);
                            ConsoleLogger.Default.LogWarning("Profile: Execution timeout reached.");
                        }
                ...
                if (this.ProfileActions?.Any() == true)
                {
                    ...
                    while (!cancellationToken.IsCancellationRequested && !timing.IsTimedOut)
                    {
                        try
                        {
                            ...
                            this.Logger.LogMessage($"{nameof(ProfileExecutor)}.ExecuteActions", LogLevel.Debug, actionExecutionContext);
        ...
        /// </summary>
        protected async Task ExecuteMonitorsAsync(EventContext parentContext, CancellationToken cancellationToken)
        {
            ...
            this.Logger.LogMessage($"{nameof(ProfileExecutor)}.ExecuteMonitors", LogLevel.Debug, monitorExecutionContext);
        ...
        protected async Task InstallDependenciesAsync(EventContext parentContext, CancellationToken cancellationToken)
        {
            if (this.ExecuteDependencies && this.ProfileDependencies?.Any() == true)
            {
                ...
                this.Logger.LogMessage($"{nameof(ProfileExecutor)}.InstallDependencies", LogLevel.Debug, dependencyInstallationContext);
