...
namespace VirtualClient
{
    ...
    public partial class ExecuteCommandMonitor : VirtualClientIntervalBasedMonitor
    {
        ...
        /// </summary>
        protected async Task ExecuteCommandWithShellSupportAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            if (this.UseShell)
            {
                ...
                switch (this.Platform)
                {
                    case PlatformID.Unix:
                        targetCommand = $"bash -c \"{targetCommand.Trim(ExecuteCommandMonitor.Quotes)}\"";
                        break;

                    case PlatformID.Win32NT:
                        targetCommand = $"cmd /C \"{targetCommand.Trim(ExecuteCommandMonitor.Quotes)}\"";
                        break;
                }
            }
