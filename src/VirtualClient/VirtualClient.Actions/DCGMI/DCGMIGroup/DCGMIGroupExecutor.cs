...
namespace VirtualClient.Actions
{
    ...
    /// <summary>
    /// The DCGMI Group Executor for GPU
    /// </summary>
    [UnixCompatible]
    public class DCGMIGroupExecutor : VirtualClientComponent
    {
        private ISystemManagement systemManagement;
        private IStateManager stateManager;

        /// <summary>
        /// Initializes a new instance of the <see cref="DCGMIGroupExecutor"/> class.
        /// </summary>
        /// /// <param name="dependencies">Provides required dependencies to the component.</param>
        /// <param name="parameters">Parameters defined in the profile or supplied on the command line.</param>
        public DCGMIGroupExecutor(IServiceCollection dependencies, IDictionary<string, IConvertible> parameters)
        ...
        /// <summary>
        /// Initializes enviroment to run DCGMI Group.
        /// </summary>
        ...
        protected override async Task InitializeAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
                ...
                switch (linuxDistributionInfo.LinuxDistribution)
                {
                    ...
                    default:
                        throw new WorkloadException(
                            $"{nameof(DCGMIGroupExecutor)} is not supported on the current Linux distro - {linuxDistributionInfo.LinuxDistribution.ToString()}.  through VC " +
            ...
            {
                throw new WorkloadException(
                            $"{nameof(DCGMIGroupExecutor)} is not supported on the current platform {this.Platform} through VC." +
        ...
        protected override async Task ExecuteAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            switch (this.Platform)
            {
                ...
                case PlatformID.Unix:
                    await this.ExecuteDCGMIGroupListCommandAsync(telemetryContext, cancellationToken)
                        .ConfigureAwait(false);
                    break;
            }
        }
        ...
        private async Task ExecuteDCGMIGroupListCommandAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            string command = "dcgmi group -l";
            this.StartTime = DateTime.Now;
            string results = await this.ExecuteCommandAsync<DCGMIGroupExecutor>(command, Environment.CurrentDirectory, cancellationToken)
            .ConfigureAwait(false);
        ...
        private void CaptureWorkloadResultsAsync(string results, DateTime startTime, DateTime endTime, EventContext telemetryContext, CancellationToken cancellationToken)
        {
            if (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    DCGMIGroupCommandParser dcgmiGroupCommandParser = new DCGMIGroupCommandParser(results);
                    IList<Metric> metrics = dcgmiGroupCommandParser.Parse();
