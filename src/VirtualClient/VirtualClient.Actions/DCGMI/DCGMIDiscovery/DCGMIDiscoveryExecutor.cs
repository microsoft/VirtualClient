...
namespace VirtualClient.Actions
{
    ...
    /// <summary>
    /// The DCGMI Discovery Executor for Nvidia GPU
    /// </summary>
    [UnixCompatible]
    public class DCGMIDiscoveryExecutor : VirtualClientComponent
    {
        private ISystemManagement systemManagement;
        private IStateManager stateManager;

        /// <summary>
        /// Initializes a new instance of the <see cref="DCGMIDiscoveryExecutor"/> class.
        /// </summary>
        /// /// <param name="dependencies">Provides required dependencies to the component.</param>
        /// <param name="parameters">Parameters defined in the profile or supplied on the command line.</param>
        public DCGMIDiscoveryExecutor(IServiceCollection dependencies, IDictionary<string, IConvertible> parameters)
        ...
        /// <summary>
        /// Initializes enviroment to run DCGMI Discovery.
        /// </summary>
        ...
        protected override async Task InitializeAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
                ...
                switch (linuxDistributionInfo.LinuxDistribution)
                {
                    ...
                    default:
                        throw new WorkloadException(
                            $"{nameof(DCGMIDiscoveryExecutor)} is not supported on the current Linux distro - {linuxDistributionInfo.LinuxDistribution.ToString()}.  through VC " +
            ...
            {
                throw new WorkloadException(
                            $"{nameof(DCGMIDiscoveryExecutor)} is not supported on the current platform {this.Platform} through VC." +
        ...
        protected override async Task ExecuteAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            switch (this.Platform)
            {
                ...
                case PlatformID.Unix:
                    await this.ExecuteDCGMIDiscoveryListCommandAsync(telemetryContext, cancellationToken)
                        .ConfigureAwait(false);
                    break;
            }
        }
        ...
        private async Task ExecuteDCGMIDiscoveryListCommandAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            string command = "dcgmi discovery -l";
            this.StartTime = DateTime.Now;

            string results = await this.ExecuteCommandAsync<DCGMIDiscoveryExecutor>(command, Environment.CurrentDirectory, cancellationToken)
            .ConfigureAwait(false);
        ...
        private void CaptureWorkloadResultsAsync(string results, DateTime startTime, DateTime endTime, EventContext telemetryContext, CancellationToken cancellationToken)
        {
            if (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    DCGMIDiscoveryCommandParser dcgmiDiscoveryCommandParser = new DCGMIDiscoveryCommandParser(results);
                    IList<Metric> metrics = dcgmiDiscoveryCommandParser.Parse();
