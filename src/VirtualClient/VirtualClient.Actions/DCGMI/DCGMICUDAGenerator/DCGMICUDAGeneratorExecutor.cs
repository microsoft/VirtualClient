...
namespace VirtualClient.Actions
{
    ...
    /// </summary>
    [UnixCompatible]
    public class DCGMICUDAGeneratorExecutor : VirtualClientComponent
    {
        private ISystemManagement systemManagement;
        private IStateManager stateManager;

        /// <summary>
        /// Initializes a new instance of the <see cref="DCGMICUDAGeneratorExecutor"/> class.
        /// </summary>
        /// /// <param name="dependencies">Provides required dependencies to the component.</param>
        /// <param name="parameters">Parameters defined in the profile or supplied on the command line.</param>
        public DCGMICUDAGeneratorExecutor(IServiceCollection dependencies, IDictionary<string, IConvertible> parameters)
        ...
        public string Level
        {
            get
            {
                this.Parameters.TryGetValue(nameof(DCGMICUDAGeneratorExecutor.Level), out IConvertible level);
        ...
        {
            await this.ExecuteCommandAsync<DCGMICUDAGeneratorExecutor>(@"nvidia-smi -pm 1", Environment.CurrentDirectory, cancellationToken)
                .ConfigureAwait(false);

            State installationState = await this.stateManager.GetStateAsync<State>(nameof(DCGMICUDAGeneratorExecutor), cancellationToken)
                .ConfigureAwait(false);
            ...
            if (installationState == null)
            {
                    ...
                    switch (linuxDistributionInfo.LinuxDistribution)
                    {
                        ...
                        default:
                            throw new WorkloadException(
                                $"{nameof(DCGMICUDAGeneratorExecutor)} is not supported on the current Linux distro - {linuxDistributionInfo.LinuxDistribution.ToString()}.  through VC " +
                                $" Supported distros include:" +
                                $" Ubuntu, Debian, CentOS8, RHEL8, SUSE",
                                ErrorReason.LinuxDistributionNotSupported);
                    }

                    await this.ExecuteCommandAsync<DCGMICUDAGeneratorExecutor>(@"nvidia-smi -e 1", Environment.CurrentDirectory, cancellationToken)
                        .ConfigureAwait(false);

                    await this.stateManager.SaveStateAsync(nameof(DCGMICUDAGeneratorExecutor), new State(), cancellationToken)
                    .ConfigureAwait(false);
                ...
                {
                    throw new WorkloadException(
                                $"{nameof(DCGMICUDAGeneratorExecutor)} is not supported on the current platform {this.Platform} through VC." +
                ...
                try
                {
                    ...
                    using (IProcessProxy process = this.systemManagement.ProcessManager.CreateElevatedProcess(this.Platform, command, $"{commandArguments}", Environment.CurrentDirectory))
                    {
                        ...
                        if (!cancellationToken.IsCancellationRequested)
                        {
                            try
                            {
                                this.Logger.LogProcessDetails<DCGMICUDAGeneratorExecutor>(process, EventContext.Persisted());
                                process.ThrowIfErrored<WorkloadException>(ProcessProxy.DefaultSuccessCodes, errorReason: ErrorReason.WorkloadFailed);
