...
namespace VirtualClient.Actions
{
    ...
    public class ElasticsearchRallyClientExecutor : ElasticsearchRallyExecutor
    {
            ...
            string rallyUserHomePath)
        {
            ...
            string rallyCommand = this.BuildRallyCommandLineArguments(
                trackName,
                raceId,
                targetHost,
                port,
            ...
            string reportPath)
        {
            ...
            if (!string.IsNullOrEmpty(this.RallyCommandLineArguments))
            {
                rallyCommand = string.Concat(rallyCommand, " ", this.RallyCommandLineArguments);
            }
        ...
        private void CaptureMetrics(string reportPath, string rallyExecutionArguments, string raceId, DateTime startTime, DateTime exitTime, EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            ElasticsearchRallyMetricsParser elasticsearchRallyMetricsParser = new ElasticsearchRallyMetricsParser(
                ...
                {
                    [nameof(this.ElasticsearchVersion)] = this.ElasticsearchVersion,
                    [nameof(this.RallyVersion)] = this.RallyVersion ?? "latest",
                    [nameof(this.RallyTrackName)] = this.RallyTrackName,
                    [nameof(raceId)] = raceId,
                },
                false);
