...
if databaseSystem == "MySQL":
    if benchmark == "TPCC":
        ...
        subprocess.run(f'sudo src/sysbench oltp_common --tables={tableCount} --table-size={recordCount} --threads={threadCount} --mysql-db={dbName} prepare', shell=True, check=True)
        if int(recordCount) == 1:
            for i in range(1,int(tableCount)+1):
                table = str(i)
                subprocess.run(f'sudo mysql -u {dbName} {dbName} -e "DROP INDEX k_{i} ON sbtest{i};"', shell=True, check=True)
