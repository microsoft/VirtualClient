...
namespace VirtualClient.UnitTests
{
    ...
    internal class CommandBaseTests : CommandBase
    {
        ...
        [TestCase(PlatformID.Win32NT, Architecture.X64)]
        [TestCase(PlatformID.Win32NT, Architecture.Arm64)]
        public void ApplicationUsesTheExpectedDefaultDirectoryLocationsWhenIsolationIsRequested(PlatformID platform, Architecture architecture)
        {
            this.SetupTest(platform, architecture);
            this.ExperimentId = Guid.NewGuid().ToString().ToLowerInvariant();
            this.Isolated = true;
            this.LogDirectory = this.mockFixture.Combine(this.mockFixture.PlatformSpecifics.CurrentDirectory, "alternate_logs_1");
            this.PackageDirectory = this.mockFixture.Combine(this.mockFixture.PlatformSpecifics.CurrentDirectory, "alternate_packages_1");
            this.StateDirectory = this.mockFixture.Combine(this.mockFixture.PlatformSpecifics.CurrentDirectory, "alternate_state_1");
            this.TempDirectory = this.mockFixture.Combine(this.mockFixture.PlatformSpecifics.CurrentDirectory, "alternate_temp_1");

            // Setup:
            // Property set when --isolated flag is used on the command line.
            string expectedExperimentId = this.ExperimentId.ToLowerInvariant();
            string expectedLogDirectory = this.mockFixture.Combine(this.LogDirectory, expectedExperimentId);
            string expectedPackagesDirectory = this.mockFixture.Combine(this.PackageDirectory, expectedExperimentId);
            string expectedStateDirectory = this.mockFixture.Combine(this.StateDirectory, expectedExperimentId);
            string expectedTempDirectory = this.mockFixture.Combine(this.TempDirectory, expectedExperimentId);

            this.EvaluateDirectoryPathOverrides(this.mockFixture.PlatformSpecifics);
            Assert.AreEqual(expectedLogDirectory, this.mockFixture.PlatformSpecifics.LogsDirectory);
            Assert.AreEqual(expectedPackagesDirectory, this.mockFixture.PlatformSpecifics.PackagesDirectory);
            Assert.AreEqual(expectedStateDirectory, this.mockFixture.PlatformSpecifics.StateDirectory);
            Assert.AreEqual(expectedTempDirectory, this.mockFixture.PlatformSpecifics.TempDirectory);
        }
        ...
        [TestCase(PlatformID.Win32NT, Architecture.X64)]
        [TestCase(PlatformID.Win32NT, Architecture.Arm64)]
        public void ApplicationUsesTheExpectedDirectoryLocationsWhenDirectoriesAreSpecifiedOnTheCommandLineAndIsolationIsRequested(PlatformID platform, Architecture architecture)
        {
            this.SetupTest(platform, architecture);
            this.ExperimentId = Guid.NewGuid().ToString().ToLowerInvariant();
            this.Isolated = true;

            string currentDirectory = this.mockFixture.PlatformSpecifics.CurrentDirectory;
            this.mockFixture.SetEnvironmentVariable(EnvironmentVariable.VC_LOGS_DIR, this.mockFixture.Combine(currentDirectory, "alternate_logs_1"));
            this.mockFixture.SetEnvironmentVariable(EnvironmentVariable.VC_PACKAGES_DIR, this.mockFixture.Combine(currentDirectory, "alternate_packages_1"));
            this.mockFixture.SetEnvironmentVariable(EnvironmentVariable.VC_STATE_DIR, this.mockFixture.Combine(currentDirectory, "alternate_state_1"));
            this.mockFixture.SetEnvironmentVariable(EnvironmentVariable.VC_TEMP_DIR, this.mockFixture.Combine(currentDirectory, "alternate_temp_1"));

            // Setup:
            // Property set when --isolated flag is used on the command line.
            string expectedExperimentId = this.ExperimentId.ToLowerInvariant();
            string expectedLogDirectory = this.mockFixture.Combine(currentDirectory, "alternate_logs_1", expectedExperimentId);
            string expectedPackagesDirectory = this.mockFixture.Combine(currentDirectory, "alternate_packages_1", expectedExperimentId);
            string expectedStateDirectory = this.mockFixture.Combine(currentDirectory, "alternate_state_1", expectedExperimentId);
            string expectedTempDirectory = this.mockFixture.Combine(currentDirectory, "alternate_temp_1", expectedExperimentId);
        ...
        [TestCase(PlatformID.Win32NT, Architecture.X64)]
        [TestCase(PlatformID.Win32NT, Architecture.Arm64)]
        public void ApplicationUsesTheExpectedDirectoryLocationsWhenDirectoriesAreSpecifiedInSupportedEnvironmentVariablesAndIsolationIsRequested(PlatformID platform, Architecture architecture)
        {
            this.SetupTest(platform, architecture);
            this.ExperimentId = Guid.NewGuid().ToString().ToLowerInvariant();
            this.Isolated = true;
            ...
            // Property set when --isolated flag is used on the command line.
            string expectedExperimentId = this.ExperimentId.ToLowerInvariant();
            string expectedLogDirectory = this.mockFixture.GetLogsPath(expectedExperimentId);
            string expectedPackagesDirectory = this.mockFixture.GetPackagePath(expectedExperimentId);
            string expectedStateDirectory = this.mockFixture.GetStatePath(expectedExperimentId);
            string expectedTempDirectory = this.mockFixture.GetTempPath(expectedExperimentId);
