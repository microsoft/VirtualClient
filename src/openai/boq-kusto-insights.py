...
cluster = "https://azurecrcdataprod.centralus.kusto.windows.net"
...
def run_kusto_query(query):
    try:
        ...
        response = client.execute("crcdb", query)
...
def authenticate_with_aad_and_create_client():
    ...
    connection_string = KustoConnectionStringBuilder.with_aad_application_token_authentication(
        f"https://azurecrcdataprod.centralus.kusto.windows.net/crcdb",
        token["accessToken"]
    )
